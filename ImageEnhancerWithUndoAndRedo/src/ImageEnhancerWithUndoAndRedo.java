 ...
 * by Xiaowen Feng for CSE 373 Assignment 1, Fall, 2016.
 * Section BB.
 * This program is an enhanced version of ImageEnhancer provided by Oracle.com and 
 * subsequently modified by S. Tanimoto, instructor for the course, which 
 * includes undo and redo features.
 * 
 * 
...
public class ImageEnhancerWithUndoAndRedo extends Component implements ActionListener {
    ...
    public ImageEnhancerWithUndoAndRedo() {
        ...
        undoStack = new BufferedImageStack(); // initialize the undoStack
        redoStack = new BufferedImageStack(); // initialize the redoStack
    }
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        // Makes a deep copy of the current buffered image
        BufferedImage biCurrent = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
        Graphics gCurrent = biCurrent.getGraphics();
        gCurrent.drawImage(biWorking, 0, 0, null);
        
        // Pushes the current buffered image to the redoStack
        if(e.getSource()==undoItem) {
        	redoStack.push(biCurrent);
        	biFiltered = undoStack.pop();
        	redoItem.setEnabled(true);
        		
        } else if (e.getSource()==redoItem){
        	undoStack.push(biCurrent);
        ...
        repaint(); // Ask Swing to update the screen.ImageEnhancerWithUndoAndRedo
    ...
    public void run() {
        JFrame f = new JFrame("ImageEnhancer WITH Undo AND Redo by Xiaowen Feng"); // Students should update this.
